...
async def test_admin_get_roles(client: AsyncClient, admin_headers: dict):
    response = await client.get("/admin/roles", headers=admin_headers)
    assert response.status_code == 200
...
async def test_admin_user_stats(client: AsyncClient, admin_headers: dict):
    response = await client.get("/admin/users/stats", headers=admin_headers)
    assert response.status_code == 200
    data = response.json()
...
async def test_admin_audit_logs(client: AsyncClient, admin_headers: dict):
    response = await client.get("/admin/audit-logs", headers=admin_headers)
    assert response.status_code == 200
...
async def test_billing_usage_authenticated(client: AsyncClient, auth_headers: dict):
    response = await client.get("/billing/usage", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
async def test_billing_subscription_info(client: AsyncClient, auth_headers: dict):
    response = await client.get("/billing/subscription", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert "plan" in data
