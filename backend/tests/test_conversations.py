...
async def test_create_conversation(client: AsyncClient, auth_headers: dict):
    response = await client.post("/conversations", json={"title": "Test Conversation"}, headers=auth_headers)
...
async def test_get_conversation(client: AsyncClient, auth_headers: dict):
    create_resp = await client.post("/conversations", json={"title": "My Chat"}, headers=auth_headers)
...
async def test_delete_conversation(client: AsyncClient, auth_headers: dict):
    create_resp = await client.post("/conversations", json={"title": "To Delete"}, headers=auth_headers)
    conv_id = create_resp.json()["id"]

    del_resp = await client.delete(f"/conversations/{conv_id}", headers=auth_headers)
...
async def test_cannot_access_other_users_conversation(
    ...
    create_resp = await client.post(
        "/conversations", json={"title": "Private"}, headers=auth_headers
    )
    conv_id = create_resp.json()["id"]
    response = await client.get(f"/conversations/{conv_id}", headers=admin_headers)
    assert response.status_code in (403, 404)


@pytest.mark.asyncio
async def test_update_conversation_title(client: AsyncClient, auth_headers: dict):
    create_resp = await client.post("/conversations", json={"title": "Old Title"}, headers=auth_headers)
    conv_id = create_resp.json()["id"]

    response = await client.patch(f"/conversations/{conv_id}", json={"title": "New Title"}, headers=auth_headers)
    assert response.status_code == 200
...
async def test_get_messages_empty(client: AsyncClient, auth_headers: dict):
    create_resp = await client.post("/conversations", json={"title": "Msg Test"}, headers=auth_headers)
    conv_id = create_resp.json()["id"]
