  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-surface-600 dark:text-gray-400 text-sm mt-1">Manage your plan, usage, and payment details</p>
      </div>
      ...
      <div className="bg-gradient-to-br from-indigo-600/20 to-violet-600/20 border border-indigo-500/30 rounded-2xl p-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-surface-600 dark:text-gray-400 text-sm">50,000 queries/month · 100GB storage · 100 users · All connectors</p>
            <p className="text-surface-500 dark:text-gray-500 text-xs mt-2">Next billing date: <span className="text-surface-600 dark:text-gray-400">April 1, 2026</span></p>
          </div>
          <div className="text-right">
            <div className="text-3xl font-bold text-white">$1,299<span className="text-lg text-surface-600 dark:text-gray-400 font-normal">/mo</span></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {usage.map(u => {
          ...
          const isHigh = pct > 80;
          return (
            <div key={u.label} className="bg-white dark:bg-gray-900 border border-surface-100 dark:border-white/5 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-surface-600 dark:text-gray-400 text-sm">{u.label}</span>
      ...
      <div className="bg-white dark:bg-gray-900 border border-surface-100 dark:border-white/5 rounded-2xl p-5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-12 h-8 bg-surface-100 dark:bg-gray-800 rounded-lg flex items-center justify-center">
              <CreditCard className="w-5 h-5 text-surface-600 dark:text-gray-400" />
            </div>
            <div>
              <div className="text-white text-sm font-medium">Visa ending in 4242</div>
              <div className="text-surface-500 dark:text-gray-500 text-xs">Expires 12/2028</div>
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-gray-900 border border-surface-100 dark:border-white/5 rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-surface-100 dark:border-white/5">
          ...
          <button className="flex items-center gap-1.5 text-sm text-surface-600 dark:text-gray-400 hover:text-white transition-colors">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-surface-100 dark:border-white/5">
              <th className="px-5 py-3 text-left text-xs font-semibold text-surface-500 dark:text-gray-500 uppercase tracking-wider">Invoice</th>
              <th className="px-5 py-3 text-left text-xs font-semibold text-surface-500 dark:text-gray-500 uppercase tracking-wider hidden sm:table-cell">Period</th>
              <th className="px-5 py-3 text-left text-xs font-semibold text-surface-500 dark:text-gray-500 uppercase tracking-wider hidden md:table-cell">Date</th>
              <th className="px-5 py-3 text-left text-xs font-semibold text-surface-500 dark:text-gray-500 uppercase tracking-wider">Amount</th>
              <th className="px-5 py-3 text-left text-xs font-semibold text-surface-500 dark:text-gray-500 uppercase tracking-wider">Status</th>
              <th className="px-5 py-3 text-right text-xs font-semibold text-surface-500 dark:text-gray-500 uppercase tracking-wider">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {INVOICES.map(inv => (
              <tr key={inv.id} className="hover:bg-surface-100/50 dark:bg-gray-800/50 transition-colors">
                <td className="px-5 py-3 text-white text-sm font-medium">{inv.id}</td>
                <td className="px-5 py-3 text-surface-600 dark:text-gray-400 text-sm hidden sm:table-cell">{inv.period}</td>
                <td className="px-5 py-3 text-surface-600 dark:text-gray-400 text-sm hidden md:table-cell">{inv.date}</td>
      ...
      <div className="bg-white dark:bg-gray-900 border border-surface-100 dark:border-white/5 rounded-2xl p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={plan.name} className={`rounded-xl p-4 border ${plan.current ? 'border-indigo-500 bg-indigo-500/10' : 'border-surface-200 dark:border-white/10 bg-surface-100/50 dark:bg-gray-800/50'}`}>
              ...
              <div className="text-2xl font-bold text-white mb-3">{plan.price}<span className="text-sm text-surface-600 dark:text-gray-400 font-normal">{plan.price !== 'Custom' ? '/mo' : ''}</span></div>
              <ul className="space-y-1.5 text-sm text-surface-600 dark:text-gray-400">
