...
const planColors: Record<string, string> = {
  ...
  Starter: 'bg-gray-500/20 text-surface-600 dark:text-gray-400 border-gray-500/30',
};
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-surface-600 dark:text-gray-400 text-sm mt-1">Manage tenant organizations across the platform</p>
        </div>
      ...
      <div className="relative max-w-sm">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search organizations..." className="w-full bg-white dark:bg-gray-900 border border-surface-200 dark:border-white/10 rounded-xl pl-9 pr-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 transition-colors" />
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-surface-100 dark:border-white/5">
              <th className="px-5 py-3 text-left text-xs font-semibold text-surface-500 dark:text-gray-500 uppercase tracking-wider">Organization</th>
              <th className="px-5 py-3 text-left text-xs font-semibold text-surface-500 dark:text-gray-500 uppercase tracking-wider hidden sm:table-cell">Plan</th>
              <th className="px-5 py-3 text-left text-xs font-semibold text-surface-500 dark:text-gray-500 uppercase tracking-wider hidden md:table-cell">Status</th>
              <th className="px-5 py-3 text-left text-xs font-semibold text-surface-500 dark:text-gray-500 uppercase tracking-wider hidden lg:table-cell">Users</th>
              <th className="px-5 py-3 text-left text-xs font-semibold text-surface-500 dark:text-gray-500 uppercase tracking-wider hidden xl:table-cell">Storage</th>
          ...
          <tbody className="divide-y divide-white/5">
            {filtered.map(org => (
              <tr key={org.id} className="hover:bg-surface-100/50 dark:bg-gray-800/50 transition-colors">
                <td className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-surface-500 dark:text-gray-500 text-xs flex items-center gap-1"><Globe className="w-3 h-3" />{org.domain}</div>
                    </div>
                  </div>
                </td>
                <td className="px-5 py-4 hidden sm:table-cell">
                  <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${planColors[org.plan] || 'bg-gray-500/20 text-surface-600 dark:text-gray-400'}`}>{org.plan}</span>
                </td>
                <td className="px-5 py-4 hidden md:table-cell">
                  <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${statusColors[org.status] || 'bg-gray-500/20 text-surface-600 dark:text-gray-400'}`}>{org.status}</span>
                </td>
                <td className="px-5 py-4 hidden lg:table-cell">
                  <span className="text-white text-sm flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-surface-500 dark:text-gray-500" />{org.users.toLocaleString()}</span>
                </td>
                <td className="px-5 py-4 text-surface-600 dark:text-gray-400 text-sm hidden xl:table-cell">{org.storage}</td>
