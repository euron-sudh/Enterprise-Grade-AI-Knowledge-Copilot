      ...
      <div className="flex gap-6">
          ...
          <nav className="space-y-1">
            {TABS.map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === tab ? 'bg-indigo-500/10 text-indigo-400' : 'text-gray-400 hover:text-white hover:bg-gray-800'}`}>
                {tab}
              </button>
            ))}
          </nav>
        ...
        <div className="flex-1">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="relative">
                  ...
                  <button className="absolute bottom-0 right-0 w-7 h-7 bg-gray-700 hover:bg-gray-600 border border-gray-600 rounded-full flex items-center justify-center transition-colors">
              ...
              <div className="space-y-4">
                  ...
                  <div key={f.label}>
                    ...
                    <div className="relative">
                      ...
                      <input defaultValue={f.value} placeholder={f.placeholder} disabled={f.disabled}
                        className="w-full pl-9 pr-4 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm placeholder-gray-600 focus:outline-none focus:border-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed" />
                    </div>
                  </div>
                ))}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select className="w-full pl-9 pr-4 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:border-indigo-500 appearance-none">
                  ...
                  return (
                    <div key={key} className="flex items-center justify-between py-3 border-b border-gray-800 last:border-0">
                      ...
                      <button onClick={() => setNotifs(n => ({ ...n, [key]: !val }))}
                        className={`relative w-11 h-6 rounded-full transition-colors ${val ? 'bg-indigo-600' : 'bg-gray-700'}`}>
          ...
          {activeTab === 'Security' && (
            <div className="space-y-4">
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2"><Shield className="w-5 h-5 text-indigo-400" />Change Password</h2>
                <div className="space-y-4">
                  ...
                  <button className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg transition-colors">Update Password</button>
                </div>
              </div>
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          ...
          {activeTab === 'Appearance' && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h2 className="text-lg font-semibold text-white mb-6 flex items-center gap-2"><Palette className="w-5 h-5 text-indigo-400" />Appearance</h2>
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {([['dark', Moon, 'Dark'], ['light', Sun, 'Light'], ['system', Monitor, 'System']] as const).map(([val, Icon, label]) => (
                    <button key={val} onClick={() => setTheme(val)}
                      className={`p-4 rounded-xl border-2 flex flex-col items-center gap-2 transition-all ${theme === val ? 'border-indigo-500 bg-indigo-500/10' : 'border-gray-700 hover:border-gray-600'}`}>
                      <Icon className={`w-6 h-6 ${theme === val ? 'text-indigo-400' : 'text-gray-400'}`} />
                      <span className={`text-sm font-medium ${theme === val ? 'text-indigo-400' : 'text-gray-400'}`}>{label}</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          {activeTab === 'API Keys' && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-lg font-semibold text-white flex items-center gap-2"><Key className="w-5 h-5 text-indigo-400" />API Keys</h2>
                  <p className="text-sm text-gray-500 mt-1">Use API keys to access KnowledgeForge programmatically.</p>
                </div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={k.name} className="flex items-center justify-between p-4 bg-gray-800 rounded-lg border border-gray-700">
                    <div>
                      <p className="text-sm font-medium text-white">{k.name}</p>
                      <p className="text-xs text-gray-500 font-mono mt-0.5">{k.key}</p>
                      <p className="text-xs text-gray-600 mt-1">Created {k.created} · Last used {k.lastUsed}</p>
                    </div>
