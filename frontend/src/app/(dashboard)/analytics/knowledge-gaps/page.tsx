    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-gray-900 border border-surface-100 dark:border-white/5 rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-surface-100 dark:border-white/5">
          ...
          <span className="text-surface-500 dark:text-gray-500 text-xs">Based on unanswered / uncited AI responses</span>
        </div>

        {loading ? (
          <div className="flex items-center justify-center py-16 text-surface-500 dark:text-gray-500">
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            {(summary?.totalAnswersWithoutSources ?? 0) === 0 && (summary?.totalAnswers ?? 0) === 0 ? (
              <>
                ...
                <p className="text-surface-500 dark:text-gray-500 text-sm">No conversations yet.</p>
                <p className="text-surface-500 dark:text-gray-600 text-xs mt-1">Start chatting with your knowledge base to surface gaps.</p>
              </>
            ) : (
              <>
                <BookOpen className="w-10 h-10 text-green-700 mb-3" />
                <p className="text-surface-600 dark:text-gray-400 text-sm font-medium">
                  {gapPct}% of answers lacked source citations
                </p>
                <p className="text-surface-500 dark:text-gray-600 text-xs mt-1">
          ...
          <div className="divide-y divide-white/5">
            {gaps.map((gap, i) => (
              <div key={gap.query} className="flex items-center gap-4 px-5 py-4 hover:bg-surface-100/50 dark:bg-gray-800/50 transition-colors">
                <span className="text-surface-500 dark:text-gray-600 text-sm w-5 text-right flex-shrink-0">{i + 1}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-white text-sm font-medium truncate">{gap.query}</div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-surface-200 dark:bg-gray-700 text-surface-600 dark:text-gray-400">{gap.category}</span>
                    <span className="text-surface-500 dark:text-gray-600 text-xs">{gap.answers} answer{gap.answers !== 1 ? 's' : ''} found</span>
                  </div>
                </div>
                <div className="flex items-center gap-3 flex-shrink-0">
                  <div className="text-right">
                    <div className="text-white text-sm font-semibold">{gap.count}</div>
                    <div className="text-surface-500 dark:text-gray-600 text-xs">queries</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="flex items-start gap-2">
              ...
              <span className="text-surface-700 dark:text-gray-300 text-sm">{rec}</span>
            </div>
          ))}
        </div>
      </div>
