    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="bg-white dark:bg-gray-900 border border-surface-200 dark:border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 bg-surface-100 dark:bg-gray-800 hover:bg-surface-200 dark:bg-gray-700 text-white font-medium px-4 py-2 rounded-xl border border-surface-200 dark:border-white/10 transition-colors text-sm disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div key={kpi.label} className="bg-white dark:bg-gray-900 border border-surface-100 dark:border-white/5 rounded-xl p-4">
            ...
            <div className="text-surface-500 dark:text-gray-500 text-xs mt-1">{kpi.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 border border-surface-100 dark:border-white/5 rounded-2xl p-5">
        ...
        {loading ? (
          <div className="flex items-center justify-center h-40 text-surface-500 dark:text-gray-500">
            <Loader2 className="w-5 h-5 animate-spin mr-2" /> Loading…
          </div>
        ) : timeSeries.length === 0 ? (
          <div className="flex items-center justify-center h-40 text-surface-500 dark:text-gray-600 text-sm">No query data yet</div>
        ) : (
          <div className="flex items-end gap-1 h-40 overflow-x-auto">
            {timeSeries.map(d => (
              <div key={d.date} className="flex-1 min-w-[24px] flex flex-col items-center gap-1">
                <span className="text-surface-500 dark:text-gray-600 text-xs hidden sm:block">{d.queries > 0 ? d.queries : ''}</span>
                <div
                ...
                <span className="text-surface-500 dark:text-gray-600 text-xs">{d.date.slice(-5)}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-900 border border-surface-100 dark:border-white/5 rounded-2xl p-5">
          ...
          {loading ? (
            <div className="flex items-center justify-center h-32 text-surface-500 dark:text-gray-500">
              <Loader2 className="w-4 h-4 animate-spin mr-2" /> Loading…
            </div>
          ) : (data?.peakHours ?? []).every(h => h.count === 0) ? (
            <div className="flex items-center justify-center h-32 text-surface-500 dark:text-gray-600 text-sm">No data yet</div>
          ) : (
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {[...timeSeries].reverse().slice(0, 7).map((d, i) => (
                <div key={d.date} className="flex items-center gap-3">
                  ...
                  <span className="text-surface-600 dark:text-gray-400 text-sm font-medium w-12 text-right">{d.queries}</span>
                </div>
              ))}
            </div>
          )}
